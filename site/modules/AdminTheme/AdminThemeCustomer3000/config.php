...
class AdminThemeCustomer3000ConfigHelper extends Wire {

	/**
	 * @var AdminThemeCustomer3000
	 * 
	 */
	 ...
	 * Construct
	 *
	 * @param AdminThemeCustomer3000 $adminTheme
	 * 
	 */
	public function __construct(AdminThemeCustomer3000 $adminTheme) {
	...
	public function configModule(InputfieldWrapper $inputfields) {
		...
		$fieldset2->description = 
			$this->_('Most advanced settings are available from the `$config->AdminThemeCustomer3000` settings array.') . ' ' . 
		...
		$f->description = $defaultFileDesc . ' ' .
			...
			$this->_('Warning: this will override custom `$config->AdminThemeCustomer3000` settings, base style and custom styles.'); 
		$f->notes = $defaultFileNote . " " .
			"[customer3000.pw.css](" . $modules->wire('config')->urls('AdminThemeCustomer3000') . "customer3000/dist/css/customer3000.pw.css)";
		$f->icon = 'file-code-o';
		$f->collapsed = Inputfield::collapsedBlank;
		$fieldset2->add($f);
